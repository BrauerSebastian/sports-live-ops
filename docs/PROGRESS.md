...
- [x] Health endpoint.
- [x] CI workflow for install, Prisma client generation, web-quality audits, lint, typecheck, unit tests, and build.
...
### Public web quality and release hardening

- [x] Privacy policy and terms pages.
- [x] Necessary-only cookie/analytics consent banner with opt-in optional analytics.
- [x] Server-only secret discipline plus automated `NEXT_PUBLIC_*` secret audit.
- [x] Production HTTPS redirect, HSTS, and baseline security headers.
- [x] Page metadata, Open Graph/Twitter preview, custom favicon, robots.txt, and sitemap.xml.
- [x] Image/alt-text audit and AVIF/WebP optimization defaults for future `next/image` assets.
- [x] Runtime Core Web Vitals reporting after consent plus static performance budgets.
- [x] Core color-contrast audit, explicit mobile viewport, and responsive public layouts.
- [x] Custom 404 and automated internal-link audit.
- [x] HTML + server-side form validation and login honeypot/rate limiting.
- [x] Clear primary Live Center CTA based on current event state.

## Verification status in this handoff environment

- [x] TypeScript `tsc --noEmit` passes.
- [x] ESLint passes.
- [x] `npm ci --package-lock-only --offline` passes, confirming npm lockfile consistency (including the Linux CI optional-dependency entries).
