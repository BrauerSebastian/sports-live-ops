...
export function RealtimePublicEvent({ eventId, context, status, homeScore, awayScore, minute, incidents, commentary, statistics, relatedNews }: { eventId: string; context: EventContext; status: EventStatus; homeScore: number; awayScore: number; minute: number; incidents: Incident[]; commentary: CommentaryItem[]; statistics: MatchStatistics; relatedNews: RelatedNewsItem[] }) {
  ...
  return <><LiveCenter context={context} status={status} homeScore={homeScore} awayScore={awayScore} minute={minute} incidents={incidents} commentary={commentary} statistics={statistics} relatedNews={relatedNews} /><p className={`realtime-state ${connection}`} role="status">{connection === "connected" ? "Live updates connected" : connection === "reconnecting" ? "Live updates reconnecting..." : "Connecting to live updates..."}</p></>;
}
