    ...
    <div className="event-workspace">
      ...
      <aside className="publish-column">
        ...
        <section className="publish-panel commentary-publish">
          ...
          <div className="composer"><textarea disabled={isBusy || !clockVisible} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Draft an update for Live Center" /><div><span>Draft only until published</span><button disabled={isBusy || !clockVisible || !draft.trim()} className="publish-button" onClick={publishCommentary}>Publish update</button></div></div>
        </section>
...
function StatisticsEditor({ statistics, onSave, pending, disabled }: { statistics: MatchStatistics; onSave: (statistics: MatchStatistics) => Promise<void>; pending: boolean; disabled: boolean }) {
  ...
  const fields = [
    { label: "Possession", key: "possession" as const },
    { label: "Shots", key: "shots" as const },
    { label: "Shots on target", key: "shotsOnTarget" as const },
    { label: "Corners", key: "corners" as const },
    { label: "Fouls", key: "fouls" as const },
  ];
  ...
  return <section className="publish-panel statistics-editor">
    ...
    {fields.map((field) => <div className="stat-edit-row" key={field.key}><span>{field.label}</span><input disabled={disabled} type="number" min="0" value={draft[field.key].home} onChange={(event) => setDraft({ ...draft, [field.key]: { ...draft[field.key], home: Number(event.target.value) } })} /><span>-</span><input disabled={disabled} type="number" min="0" value={draft[field.key].away} onChange={(event) => setDraft({ ...draft, [field.key]: { ...draft[field.key], away: Number(event.target.value) } })} /></div>)}
