...
export async function generateMetadata({ params }: { params: Promise<{ competitionId: string }> }): Promise<Metadata> {
  try {
    ...
    return competition
      ? { title: `${competition.name} fixtures and results`, description: `Fixtures, live matches and results for ${competition.name}.` }
      : { title: "Fixtures and results" };
  } catch {
    return { title: "Fixtures and results" };
  }
}
  ...
  return (
    <main className="public-main">
      <div className="public-intro">
        <div><p className="overline">{competition.name}</p><h1>Fixtures and results</h1><p className="event-location">{competition.seasons[0]?.name}</p></div>
        <Link className="secondary-button" href={`/live/competitions/${competition.id}`}>Competition overview</Link>
      </div>
      <div className="filter-bar" aria-label="Fixture filters">
        {filters.map((filter) => (
          <Link className={selected === filter.value || (!selected && !filter.value) ? "filter-active" : ""} href={filter.value ? `?status=${filter.value}` : "?"} key={filter.label}>
            {filter.label}
          </Link>
        ))}
      </div>
        ...
        {events.length ? events.map((event) => {
          ...
          return (
            <Link className="public-event-row" href={`/live/events/${event.id}`} key={event.id}>
              ...
              <span className={isLive ? "public-live-status" : ""}>{isLive && <span className="live-dot" aria-hidden="true" />}{state}</span>
            </Link>
          );
