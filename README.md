...
## Web quality and release hardening

The public surface includes privacy and terms pages, opt-in analytics consent, custom metadata/social previews/favicon, `robots.txt`, `sitemap.xml`, HTTPS enforcement in production, security headers, a custom 404, responsive layouts, form validation, login anti-abuse controls, Core Web Vitals reporting, and source-level quality audits.
...
This executes internal-link, image/alt-text, frontend-secret, contrast, and performance-budget checks. See [`docs/WEB-QUALITY.md`](docs/WEB-QUALITY.md) for the implementation details and the remaining deployment-level Lighthouse check.
